...
beautiful.init("/home/cyber/.config/awesome/themes/default/theme.lua")
...
function switch()
  ...
  local t = layouts[current]
  os.execute(string.format("setxkbmap %s %s", t[1], t[2]))
end

mykeyboardlayout:connect_signal(
  'button::press',
  function(widget, _, __, button)
    if button == 1 then
      switch()
    end
  end)
...
function dir (s, n, f)
  ...
  if f ~= io.stdout or f ~= io.stderr or f ~= io.stdin then f:close() end
end
...
function net_widget(name, timeout)
    local widg =
        awful.widget.watch(
        string.format("%s %s %s", "sh", script("net"), name or ""),
        timeout or 2,
        function(widget, stdout)
            -- Put each value into table
            local lines = {}
            ...
            local text = string.format("%s 🔻%s 🔺%s", name or "All", formatBytes(lines[1]), formatBytes(lines[2]))
